...
// import { Search, Star, Eye, Plus, Loader, X } from "lucide-react";
...
//                       setPriceRange((prev) => ({
//                         ...prev,
//                         max: Infinity, // Reset to Infinity to remove filter
//                       }))
//                     }
...
import React, { useState, useMemo, useEffect } from "react";
import { Link } from "react-router-dom";
import { Search, Star, Eye, Loader, Loader2 } from "lucide-react";
import { useLocation, useNavigate } from "react-router-dom";
...
const ProductList: React.FC = () => {
  ...
  // Basic filter state (keep search term for regular product filtering)
  const [searchTerm, setSearchTerm] = useState<string>("");
  ...
  // Filter products client-side (only by basic search term now)
  const filteredProducts = useMemo(() => {
    return products.filter((product) => {
      const matchesSearch =
        product.name?.toLowerCase().includes(searchTerm.toLowerCase()) ||
    ...
    <div className="min-h-screen bg-gray-50 py-8">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          {pagination && (
            <p className="text-sm text-gray-500 mt-1">
              Showing {showSearchResults ? searchResults.length : filteredProducts.length} of {showSearchResults ? searchResults.length : products.length} products
                ...
                >
                  ...
                  <div className="p-4">
                    ...
                    <div className="text-sm text-gray-600 mb-2">
                      <div>{result.ram} RAM • {result.storage} Storage</div>
                      <div>{result.camera} • {result.battery}</div>
                    </div>
                    <div className="flex items-center justify-between">
